""" PyTorch PLF MAE (masked autoencoder) model."""
...
@dataclass
class PLFModelOutput(ModelOutput):
    """
    Class for PLFModel's outputs, with potential hidden states and attentions.
...
@dataclass
class PLFDecoderOutput(ModelOutput):
    """
    Class for PLFDecoder's outputs, with potential hidden states and attentions.
...
@dataclass
class PLFForPreTrainingOutput(ModelOutput):
    """
    Class for PLFForPreTraining's outputs, with potential hidden states and attentions.
...
@dataclass
class PLFClassifierOutput(ModelOutput):
    """
...
@dataclass
class PLFJointClassificationRegressionOutput(ModelOutput):
    """
    """
    loss: Optional[torch.FloatTensor] = None
...
class PLFEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.patch_embeddings = PLFPatchEmbeddings(config)
...
class PLFPatchEmbeddings(nn.Module):
    """
...
class PLFSelfAttention(nn.Module):
    def __init__(self, config: PLFConfig) -> None:
        super().__init__()
...
class PLFSelfOutput(nn.Module):
    """
    The residual connection is defined in PLFLayer instead of here (as is the case with other models), due to the
...
class PLFAttention(nn.Module):
    def __init__(self, config: PLFConfig) -> None:
        super().__init__()
        self.attention = PLFSelfAttention(config)
        self.output = PLFSelfOutput(config)
...
class PLFIntermediate(nn.Module):
    def __init__(self, config: PLFConfig) -> None:
        super().__init__()
...
class PLFLayer(nn.Module):
    ...
    def __init__(self, config: PLFConfig) -> None:
        ...
        self.seq_len_dim = 1
        self.attention = PLFAttention(config)
        self.intermediate = PLFIntermediate(config)
        self.output = PLFOutput(config)
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(
            self.layernorm_before(hidden_states),  # in PLF, layernorm is applied before self-attention
            head_mask,
        ...
        # in PLF, layernorm is also applied after self-attention
...
class PLFEncoder(nn.Module):
    def __init__(self, config: PLFConfig) -> None:
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([PLFLayer(config) for _ in range(config.num_hidden_layers)])
...
class PLFPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = PLFConfig
    base_model_prefix = "plf"
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, PLFEncoder):
            module.gradient_checkpointing = value


class PLFModel(PLFPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = PLFEmbeddings(config)
        self.encoder = PLFEncoder(config)
    ...
    def forward(
        ...
        r"""
            ...
            ```python
            >>> from transformers import AutoFeatureExtractor, PLFModel
            ...
            >>> feature_extractor = AutoFeatureExtractor.from_pretrained("facebook/plf-mae-base")
            >>> model = PLFModel.from_pretrained("facebook/plf-mae-base")
...
class PLFDecoder(nn.Module):
    def __init__(self, config, num_filtration_para):
        ...
        self.decoder_layers = nn.ModuleList(
            [PLFLayer(decoder_config) for _ in range(config.decoder_num_hidden_layers)]
        )
...
class PLFForPreTraining(PLFPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.plf = PLFModel(config)
        self.decoder = PLFDecoder(config, num_filtration_para=self.plf.embeddings.num_filtration_para)

        # Initialize weights and apply final processing
        self.post_init()

    def get_input_embeddings(self):
        return self.plf.embeddings.patch_embeddings
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, PLFForPreTrainingOutput]:
        r"""
            Returns:
            ...
            ```python
            >>> from transformers import AutoFeatureExtractor, PLFForPreTraining
            >>> from PIL import Image
            >>> import requests
            ...
            >>> feature_extractor = AutoFeatureExtractor.from_pretrained("facebook/plf-mae-base")
            >>> model = PLFForPreTraining.from_pretrained("facebook/plf-mae-base")
        ...
        return PLFForPreTrainingOutput(
            loss=loss,
            logits=logits,
            mask=mask,
            ids_restore=ids_restore,
...
class PLFPooler(PLFPreTrainedModel):
    def __init__(self, config: PLFConfig):
        super().__init__(config)
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
...
class PLFForImageClassification(PLFPreTrainedModel):
    def __init__(self, config: PLFConfig) -> None:
        super().__init__(config)

        self.num_labels = config.num_labels
        self.plf = PLFModel(config)
        self.pooler = PLFPooler(config)
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[tuple, PLFClassifierOutput]:
        r"""
...
class PLFForJointClassificationRegression(PLFPreTrainedModel):
    def __init__(self, config: PLFConfig) -> None:
        super().__init__(config)
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[tuple, PLFJointClassificationRegressionOutput]:
        r"""
        labels (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            Labels for computing the image classification/regression loss. Indices should be in `[0, ...,
            config.num_labels - 1]`. If `config.num_labels == 1` a regression loss is computed (Mean-Square loss), If
        ...
        return PLFJointClassificationRegressionOutput(
            loss=loss,
...
class PLFFForClassificationReconstruction(PLFPreTrainedModel):
