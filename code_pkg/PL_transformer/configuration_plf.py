...
class PLFConfig(PretrainedConfig):
    r"""
        This is the configuration class to store the configuration of a [`PLFModel`]. It is used to instantiate an PLF
        MAE model according to the specified arguments, defining the model architecture. Instantiating a configuration with
        the defaults will yield a similar configuration to that of the PLF
        [facebook/plf-mae-base](https://huggingface.co/facebook/plf-mae-base) architecture.
        ...
        ```python
        >>> from transformers import PLFModel, PLFConfig

        >>> # Initializing a PLF MAE plf-mae-base style configuration
        >>> configuration = PLFConfig()

        >>> # Initializing a model from the plf-mae-base style configuration
        >>> model = PLFModel(configuration)
